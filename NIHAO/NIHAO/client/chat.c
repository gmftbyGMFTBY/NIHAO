...
//
GtkWidget *group_listbox;

gchar username[20];
gint useravator;
gchar cur_chat_friend_name[20];
gchar cur_chat_group_name[20];
...
FriendItem *utalk_frienditem_new (const gchar *friend_name, GtkWidget *friendbox, Msg_queue *msg_queue, int type){
...
void add_friend (const char *friend_name, gint avator_id, const char *msg, int position){
	...
	FriendItem *frienditem = utalk_frienditem_new (friend_name, friendbox_with_msg, msg_queue, 1);
	friends_list = g_list_insert (friends_list, frienditem, position);
}

void add_group(const char* group_name, gint avator_id, const char* msg, int position)
{
    gchar path[10];
    sprintf(path, "group_avators/%d.jpg", avator_id);
    GtkWidget *friendbox_with_msg = utalk_friendbox_with_msg_new(path, group_name, msg);
    gtk_list_box_insert((GtkListBox *)group_listbox, friendbox_with_msg, position);

    Msg_queue *msg_queue = Msg_queue_new ();
    Message *message = Message_new (group_name, msg);
    FriendItem *groupitem = utalk_frienditem_new (group_name, friendbox_with_msg, msg_queue, 0);
    group_list = g_list_insert (group_list, groupitem, position);
}
...
void friend_selected (GtkListBox *box, GtkListBoxRow *row, gpointer user_data){
	...
	strcpy (cur_chat_group_name, name);
	...
	for (GList *l = friends_list; l != NULL; l = l->next){
		...
		if (strcmp(frienditem->friend_name, name) == 0)
        {
            f = 1;
            break;
        }
    ...
    {
        is_group = 1;
        for (GList *l = group_list; l != NULL; l = l->next){
            frienditem = (FriendItem *)(l->data);
            if (strcmp(frienditem->friend_name, name) == 0)
            {
                break;
...
void recv_gmsg (const char* groupname, const char *friendname, const char *msg){
    FriendItem *frienditem = NULL;
    for (GList *l = group_list; l != NULL; l = l->next){
        frienditem = (FriendItem *)(l->data);
        if (strcmp(frienditem->friend_name, groupname+1) == 0) break;
    }
    if (frienditem == NULL){
        server_error_dialog (window);
        return;
    ...
    if (strcmp(groupname+1, cur_chat_group_name) == 0){
        if(strcmp(friendname, username)!=0) {
            update_msg_textview(friendname, msg);
        }
    }
    else{
...
void send_button_clicked (GtkWidget *widget, gpointer data){
	...
	int succeed = send_msg (cur_chat_friend_name, msg, is_group);
	if (succeed == 0){
		server_error_dialog (window);
		return;
	}
...
//
void load_group_list_from_server(){
	//Clear
	group_listbox = GTK_WIDGET(gtk_builder_get_object(builder, "group_listbox"));
	GList *list = gtk_container_get_children((GtkContainer*)group_listbox);
	for(GList *l = list;l != NULL;l = l->next) {
        gtk_container_remove ((GtkContainer *)group_listbox, (GtkWidget *)(l->data));
	}

	int photo_id[MAX_NUM];
	char group_name_list[MAX_NUM][MAX_NUM];
	int group_num = load_group_list(group_name_list, photo_id);
    g_print ("Load friends list done. friends counting %d.\n", group_num);
    g_print ("in chat: %s\n", group_name_list[0]);

    for(int i = 0; i < group_num; i++)
    {
        add_group(group_name_list[i], photo_id[i], "", -1);
    }
    gtk_widget_show_all(group_listbox);
}
//
...
void chat_init (){
	...
	g_signal_connect (G_OBJECT(group_listbox), "row-selected", G_CALLBACK(friend_selected), NULL);
...
GtkWidget *load_chat_window (const char *recv_username){
	builder = gtk_builder_new_from_file ("/home/steven/desktop/uTalk/uTalk-final/client/chat.glade");
	if(builder == NULL) return NULL;
	
	window = GTK_WIDGET (gtk_builder_get_object(builder, "window"));
	gtk_window_set_title(GTK_WINDOW(window),"NIHAO");
