...
int send_msg(const char* friendname, const char* massage, int is_group);
int load_friends_list(char friendname[][21], int *photo);
//
int load_group_list(char groupname[][21], int* photo);
//
...
int signin(const char* username, const char* password)
{
    ...
    if ((rlen = recv(socket_fd, recv_message, MAXN, 0)) == -1) 
    { 
...
int load_friends_list(char friendname[][21], int *photo)
{
 ...
 int load_group_list(char groupname[][21], int* photo)
{
    char send_message[MAXN];
    sprintf(send_message, "/9:%s*", currect_name);
    
    send(socket_fd, send_message, strlen(send_message)+1, 0);
    
    usleep(1000000);
    
    for(int i=0;i<n;i++)
    {
        strcpy(groupname[i], group[i]);
        photo[i] = gn_photo[i];
        puts(groupname[i]);
    }
    return n;
...
void *send_files(void *ip)
{
    ...
    while((slen = fread(send_massage, sizeof(char), 4000, fp)) > 0)
    {
        ...
        //例行回复
        read(socket_fd, recv_message, MAXN);
    }
...
void *recv_files(void *ip)
{
	...
	perror("fopen");
    while((rlen = read(connect_fd, recv_message, MAXN)) > 0)
    {
        //收到结束信号，退出
        if(strcmp(recv_message, "finash") == 0)
        {
            //告知对方收到
            write(connect_fd, "ok", 3);
            break;
        }
        fwrite(recv_message, sizeof(char), rlen, fp);
...
void *recving (void *p_null)
{
    ...
    while(1)
    {
        rlen = recv(socket_fd, recv_message, MAXN, 0);
        puts(recv_message);
        ...
        if(recv_message[0] == '/')
        {
            ...
            if(recv_message[1] == '2' || recv_message[1] == '5')
            {
                FILE *fp = fopen("temp", "w");
                fprintf(fp, "%s", recv_message + 3);
                fclose(fp);
            ...
            else if(recv_message[1] == '3')
            {
                ...
                if(recv_message[3] == '3')
                {
                    sscanf(recv_message, "/3:3%s", friendname);
                    //friend_request_response(friendname, 1);
                }
                //refuse
                else if(recv_message[3] == '2')
                {
                    sscanf(recv_message, "/3:2%s", friendname);
                    //friend_request_response(friendname, 0);
                }
                //request
                else
                {
                    sscanf(recv_message, "/3:%s", friendname);
                    sprintf(recv_message, "/3:1%s*", friendname);
                    send(socket_fd, recv_message, strlen(recv_message) + 1, 0);
                    //recv_friend_request(friendname);
                }
            }
            //grouplist;
            else if (recv_message[1] == '4')
            {
                FILE *fp = fopen("temp", "w");
                fprintf(fp, "%s", recv_message + 3);
                fclose(fp);
                ...
                for(i = 0; i < n; i++)
                {
                    fscanf(rp, "%s %d", group[i], gn_photo + i);
                }
                fclose(rp);
            }
            else if (recv_message[1] == '7')
            {
                ;
            }
            else if (recv_message[1] == '8')
            {
                int photo_id;
                sscanf(recv_message, "/8:%d*", &photo_id);
        ...
        {
            puts(filename);
            sscanf (recv_message + 1, "%s%s", filename, friendname);
            FILE *fp;
            ...
            fp = fopen(filename, "a");
            fwrite(recv_message + 15, sizeof(char), rlen - 15, fp);
            fclose(fp);
            if(rlen < 100)
            {
        ...
        else if(recv_message[0] == '$')
        {
             ...
             sscanf(recv_message, "%s %s %s", groupname, friendname, massage);

             sscanf(massage, "%s", judge);

             recv_gmsg(groupname, friendname, massage);
        }
        else
        {
            for(i = 0; i >= 0; i++)
            ...
            sscanf(recv_message, "%s %s", friendname, massage);

            sscanf(massage, "%s", judge);
            if (strcmp(judge, "sendfile") == 0)
            {
                printf("recv_message: %s\n", recv_message);
                sscanf(recv_message, "%s%s%s%s", n_friendname, judge, n_filename, ip);                
